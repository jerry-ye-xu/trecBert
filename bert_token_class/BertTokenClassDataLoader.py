...
class BertTokenClassDataHolder():
    def __init__(
        self,
        model_name, config, tag2id_dict,
        hf_token_class=BertTokenizer,
        vocab_file=None):
        """

        BertTokenClass is a sequence labelling exercise, we first convert all possible NER tokens to a map, and just like the input text data.

        Hence when we are constructing the data for DataLoader, we'll need to be pass in the labels as well, but map them to a dictionary.
        ...
        self.tag2id_dict = tag2id_dict
        self.tag2id_dict["PAD_LABEL_TOKEN"] = -100
        self.id2tag_dict = {v: k for k, v in self.tag2id_dict.items()}
        self.id2tag_dict[-100] = "PAD_LABEL_TOKEN"
        self.num_labels = len(tag2id_dict.keys())
    ...
    def _truncate_seq_pair(self, pair_a, pair_b, label):
        """Truncates a sequence pair to the maximum length."""

        if pair_b is None:
            if len(pair_a) > (self.max_token_len - 2):
                return pair_a[:(self.max_token_len - 2)], label[:(self.max_token_len - 2)], pair_b
        else:
            while True:
        ...
        return pair_a, label, pair_b

    def _tokenize_seq(self, pair_a, pair_b, label):

        pair_a, label_ab, pair_b = self._truncate_seq_pair(pair_a, pair_b, label)

        # print(f"pair_a: {pair_a}")
        pair_a = ["[CLS]"] + pair_a + ["[SEP]"]
        seg_ids_a = [0] * len(pair_a)

        label_tokens = ["[CLS]"] + label + ["[SEP]"]

        if pair_b is not None:
            pair_b = pair_b + ["[SEP]"]
            seg_ids_b = [1] * len(pair_b)

            label_tokens.insert(len(pair_a) - 1, "[SEP]")
        ...
        return pair_ab_token_id, seg_ids, input_mask, label_tokens

    def _build_tokenised_dataset_seq(self, seq_a, seq_b, label_seq):
        # seq_a_tokenised = [self.tokenizer.tokenize(s) for s in seq_a]

        # label_seq_tokenised = [label_dict[tag] for tag in label_seq]

        seq_a_tokenised, label_seq_a = expand_label_tokens(
            seq_a, label_seq[:len(seq_a)]
        )
        ...
        if seq_b is not None:
            seq_a_tokenised, label_seq_b = expand_label_tokens(
                seq_a, label[len(seq_a):]
            )
            label_seq_tokenised = [label_dict[tag] for tag in label_seq_a + label_seq_b]

            for pair_a, pair_b, label_ab in tqdm(zip(seq_a_tokenised, seq_b_tokenised, label_seq_tokenised), desc="SEQ_A_and_B"):
                pair_ab_token_id, seg_ids, input_mask, label_tokens = self._tokenize_seq(pair_a, pair_b, label_ab)
            ...
            label_seq_tokenised = [label_dict[tag] for tag in label_seq_a]
            for pair_a, label_a, in tqdm(zip(seq_a_tokenised, label_seq_tokenised), desc="SEQ_A"):
                pair_a_token_id, seg_ids, input_mask, label_tokens = self._tokenize_seq(pair_a, None)
        ...
        return input_ids, token_type_ids, attention_masks, label_ids

    def extend_label_tokens(seq, label):
        seq_wordpiece = []
        label_expand = []
        for s, l in zip(seq, label):
            s_wp = self.tokenizer.tokenize(s)
            seq_wordpiece.extend(s_wp)
            label_expand.extend([label] + ["PAD_LABEL_TOKEN"]*(len(s_wp) - 1))

        return seq_wordpiece, label_expand
    ...
    def create_train_and_test_dataset(
        ...
        if split_by_attribute is not None:
            ...
            input_ids, token_type_ids, attention_masks, labels_ids = self._build_tokenised_dataset_seq(seq_a, seq_b, labels)
...
print("BertTokenClassDataLoader refreshed")
