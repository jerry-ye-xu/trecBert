...
class BertForTokenFinetune():
    def __init__(
        self,
        model_name, config, num_labels, model_desc,
        save_ckpt_path, save_flag,
        hf_model_class=BertForTokenClassification,
    ...
    def train(self, epochs, batch_size, use_gpu):
        ...
        for epoch in trange(epochs, desc="EPOCHS"):
            ...
            if self.save_flag:
                ...
                # @TODO: Fix evaluate function first.
                if (epoch % 1) == 0:
                    pass
    ...
    def evaluate(self, eval_metrics_dict, use_ids, validate, use_gpu):
        ...
        for batch in tqdm(test_data, desc="EVALUATING"):
            ...
            if not validate:
                labels_batch = inputs["labels"].detach().cpu().numpy()
...
print("BertTokenClassFinetune refreshed")
