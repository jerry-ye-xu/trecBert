...
def process_baseline_raw(
    ...
    df_input["topic_year"] = df_input[global_var["year"]].astype(str) + "_" + df_input[global_var["topic"]].astype(str)
    print("Inside process_baseline_raw(), checking for topic_year mapping, all years present. Data will be split into training and testing later.")
    print(df_input["topic_year"].iloc[:5])

    ty_id = sorted(df_input["topic_year"].unique())

    ty_id_map = dict(zip(ty_id, [i for i in range(len(ty_id))]))
    print(ty_id_map)

    # Add this to ID
    df_input[global_var["attrib_seq"]] = df_input["topic_year"].map(ty_id_map)
    print(df_input[[
              global_var["topic"],
              global_var["year"],
              "topic_year",
...
def split_into_train_test_set(
    ...
    use_qe: bool,
    global_var: Dict[str, Any]) -> List[Any]:
