...
from bert_seq_sent.customDatasetGenerator import BioASQDatasetGenerator, clinicalDatasetGenerator, id_collate
from bert_seq_sent.evalFunc import num_correctly_classified, calc_roc

class BertForSentFinetune():
    def __init__(
        self,
    ...
    def train(self,
        ...
        # UPDATE when using multiple data sources
        training_data = BioASQDatasetGenerator(
            self.model_name,
            self.config,
            self.vocab_file,
            self.num_labels,
        ...
        self.training_data_gen = DataLoader(
            training_data,
        ...
        self.total_steps = len(self.training_data_gen) // (self.gradient_accumulation_steps * epochs)
        ...
        print(f"len(self.training_data_gen): {len(self.training_data_gen)}")
        # total_iters = len(self.training_data_gen)//batch_size

        for epoch in trange(epochs, desc="EPOCHS"):
            for batch, qa_id in tqdm(self.training_data_gen,
                                     desc="FINETUNING"):
                                    #  total=len(self.training_data_gen)):
    ...
    def evaluate(self,
        ...
        Format of the batch is different depending on use_topics:

        IF use_topics IS True:
        (
          [
            tensor([
              [a1], ..., [an]
        ...
        use_topics: We doing sentence-level evaluation, we MUST use_topics, and thus use_topics in this case signals whether we should be using topic_ids for clinical trials data (if we are just evaluating on the bioasq data then we don't need to use topics.)
        validate: If true, then this is a validation set with no labels.
        specify_dataset: ["clinical", "bioasq"]
        """
        eval_loss = 0.0
        nb_eval_steps = 0
        ...
        if specify_dataset == "clinical":
            test_data = clinicalDatasetGenerator(
                self.model_name,
                self.config,
                self.vocab_file,
                self.num_labels,
        ...
        elif specify_dataset == "bioasq":
            test_data = BioASQDatasetGenerator(
                self.model_name,
                self.config,
                self.vocab_file,
                self.num_labels,
        ...
        self.testing_data_gen = DataLoader(
            test_data,
            batch_size=batch_size,
            shuffle=True,
            num_workers=2,
        ...
        print(f"len(self.testing_data_gen): {len(self.testing_data_gen)}")
        # total_iters = len(self.testing_data_gen) // batch_size

        for batch in tqdm(self.testing_data_gen,
                          desc="EVALUATING"):
            self.model.eval()

            with torch.no_grad():
                # print(batch)
                # print(f"doc_ids_batch: {doc_ids_batch}")
                doc_ids_batch = batch[1]
                if use_topics:
                    # print(f"input_ids: {batch[0][0]}")
            ...
            # print(f"self.topics_eval_arr: {self.topics_eval_arr}")
        ...
        num_test_points = len(self.testing_data_gen.dataset)
        # if validate:
        ...
        return self.labels_arr, self.preds_arr, self.topics_eval_arr, self.doc_id_eval_arr, self.validation_accuracy
